...
static pthread_mutex_t m_t_MutexCond[DRV_UART_DEVICE_ID_COUNT] =
	PTHREAD_MUTEX_INITIALIZER;
static pthread_mutex_t m_t_MutexEvent[DRV_UART_DEVICE_ID_COUNT] =
	PTHREAD_MUTEX_INITIALIZER;
static pthread_cond_t m_t_Cond[DRV_UART_DEVICE_ID_COUNT] =
...
uint TaskComm_Initialize(void)
{
	...
	{
		if (LibQueue_Initialize(&m_t_QueueSend[i], m_u8_BufferQueue[i],
			sizeof(m_u8_BufferQueue[i])) != FUNCTION_OK)
		{
			LOGD("Initialization fail 3333");
			return FUNCTION_FAIL;
		}

		REG_SET_BIT(m_ui_State[i], TASK_COMM_STATE_INITIAL);
		m_ui_Address[i] = i;

		if (pthread_create(&t_ThreadID, NULL, TaskComm_WriteThread,
			(void *)&m_ui_Address[i]) != 0)
		{
...
void TaskComm_Finalize(void)
{
	...
	for (i = 0; i < DRV_UART_DEVICE_ID_COUNT; i++)
	{
		REG_CLEAR_BIT(m_ui_State[i], TASK_COMM_STATE_INITIAL);
	}
}
...
)
{
	...
	switch (ui_Parameter)
	{
		...
		case TASK_COMM_PARAM_LINK:
			...
			if (TaskComm_GetDevice((devcomm_int)ui_Address, &t_Device) !=
				FUNCTION_OK)
			{
				return FUNCTION_FAIL;
			}
			...
			{
				return DevComm_Link(t_Device, TASK_ADDRESS_MASTER,
...
)
{
	...
	switch (ui_Parameter)
	{
		case TASK_COMM_PARAM_BUSY:
			...
			if (LibQueue_GetConfig(&m_t_QueueSend[t_Device],
			...
			if (DevComm_Query(t_Device, TASK_ADDRESS_MASTER, DEVCOMM_INFO_STATE,
				&t_Value) != FUNCTION_OK)
			{
				return FUNCTION_FAIL;
			}
...
)
{
	...
	if (LibQueue_PushTail(&m_t_QueueSend[t_Device],
	...
	REG_SET_BIT(m_ui_State[t_Device], TASK_COMM_STATE_SENDING);
...
)
{
	...
	if (DevComm_Receive(t_Device, TASK_ADDRESS_MASTER, &t_SourcePort,
		&t_TargetPort, m_u8_BufferCommand[t_Device], &t_Length, &t_Mode) !=
		FUNCTION_OK)
	{
		return FUNCTION_FAIL;
	}
	...
	*u8p_Mode = (uint8)t_Mode;
	tp_Command->u8_Operation =
		m_u8_BufferCommand[t_Device][TASK_COMM_COMMAND_OFFSET_OPERATION];
	tp_Command->u8_Parameter =
		m_u8_BufferCommand[t_Device][TASK_COMM_COMMAND_OFFSET_PARAMETER];
	tp_Command->u8_Length = (uint8)t_Length - TASK_COMM_COUNT_COMMAND_OFFSET;
	tp_Command->u8p_Data = &m_u8_BufferCommand[t_Device][TASK_COMM_COUNT_COMMAND_OFFSET];
...
static uint TaskComm_InitializeDevice(void)
{
	...
	for (i = 0; i < DRV_UART_DEVICE_ID_COUNT; i++)
	{
		if (DrvUART_SetConfig(i, DRV_UART_PARAM_CALLBACK,
			(const void *)&t_Callback) != FUNCTION_OK)
		{
			return FUNCTION_FAIL;
		}
	}
...
static uint TaskComm_InitializeComm(void)
{
	...
	for (i = 0; i < DRV_UART_DEVICE_ID_COUNT; i++)
	{
		...
		if (DevComm_Link(i, TASK_ADDRESS_MASTER,
	...
	do
	{
		if (t_Event == TASK_COMM_EVENT_RECEIVE_DONE)
		{
			...
			t_Command.u8_Parameter =
				m_u8_BufferCommand[t_Device][TASK_COMM_COMMAND_OFFSET_PARAMETER];
			t_Command.u8_Length =
				(uint8)t_Length - TASK_COMM_COUNT_COMMAND_OFFSET;
			t_Command.u8p_Data =
...
)
{
	...
	if (DevComm_ReadDeviceDone((devcomm_int)ui_DeviceID, u8p_Data,
...
{
	return m_t_Callback.fp_WriteDevice(m_ui_Address[ui_DeviceID], u8p_Data,
		u8_Length);
}
...
)
{
	...
	if (LibQueue_PopHead(&m_t_QueueSend[t_Device], (uint8 *)&t_MessageHead,
	...
	if (ui_Value > 0)
	{
		...
		if (LibQueue_PopHead(&m_t_QueueSend[t_Device],
	...
	if (DevComm_Send(t_Device, TASK_ADDRESS_MASTER,
		(devcomm_int)t_MessageHead.u8_SourcePort,
		(devcomm_int)t_MessageHead.u8_TargetPort,
		(const uint8 *)m_u8_BufferCommand[t_Device],
...
static void* TaskComm_WriteThread(void* arg)
{
	...
	while (REG_GET_BIT(m_ui_State[t_Device], TASK_COMM_STATE_INITIAL) != 0)
	{
		pthread_mutex_lock(&m_t_MutexCond[t_Device]);

		if (REG_GET_BIT(m_ui_State[t_Device], TASK_COMM_STATE_SENDING) == 0)
		{
			pthread_cond_wait(&m_t_Cond[t_Device], &m_t_MutexCond[t_Device]);
		}

		REG_CLEAR_BIT(m_ui_State[t_Device], TASK_COMM_STATE_SENDING);
		pthread_mutex_unlock(&m_t_MutexCond[t_Device]);
